...
def in_any_session(utc_index: pd.DatetimeIndex, specs: list[tuple[str,str,str]]) -> pd.Series:
    """
    specs: list of (tzname, startHH:MM, endHH:MM). End inclusive.
    Handles DST correctly via dateutil tz.
    """
    mask = pd.Series(False, index=utc_index)
    for tzname, start_s, end_s in specs:
        tzinfo = tz.gettz(tzname)
        start_h, start_m = map(int, start_s.split(":"))
        end_h, end_m = map(int, end_s.split(":"))
        # convert to local time-of-day in that timezone
        local = utc_index.tz_convert(tzinfo)
...
def cmd_sessions(args):
    ...
    if df.index.tzinfo is None:
        df.index = df.index.tz_localize("UTC")
    specs = []
    # args.name is repeated; zipped with tz/start/end
    for name, tzname, start, end in zip(args.name, args.tz, args.start, args.end):
        specs.append((tzname, start, end))
    mask = in_any_session(df.index, specs)
    if args.filter:
        out = df[mask].copy()
    else:
        out = df.copy()
        out["in_session"] = mask.values
    Path(args.out).parent.mkdir(parents=True, exist_ok=True)

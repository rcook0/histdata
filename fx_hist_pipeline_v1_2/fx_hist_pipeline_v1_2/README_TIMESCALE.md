...
-- same pattern; the timezone handles DST shifts.
```

Gap-aware backtests (skip windows with missing bars):
```sql
-- Expect 60 bars per hour. Flag hours with < 58 real bars.
WITH hours AS (
  SELECT symbol,
  ...
  FROM fx_m1
  WHERE symbol = 'EURUSD'
    AND ( (ts AT TIME ZONE 'Europe/London')::time BETWEEN TIME '08:00' AND TIME '17:00' )
  GROUP BY 1,2
)
SELECT *
FROM hours
...
FROM fx_m1
WHERE symbol='EURUSD'
  AND ( (ts AT TIME ZONE 'America/New_York')::time BETWEEN TIME '08:00' AND TIME '17:00' )
